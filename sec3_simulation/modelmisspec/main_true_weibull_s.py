...
from mmm_class import MMM_NoAdstock
from util import (generate_exogenous_data, generate_sales, compute_true_roi, 
                  create_model_specs_no_adstock)


# Custom data generation for Weibull S-shape
def generate_sales_weibull_s(spend, data, params, seed=None):
    """Generate sales using Weibull S-shape saturation + adstock"""
    if seed is not None:
        np.random.seed(seed)
    ...
    def weibull_s_saturation(z, K, S):
        """Weibull S-shape (S > 1)"""
    ...
    for l in range(L):
        for t in range(T):
            ...
            for c in range(C):
                ...
                spend_sat = weibull_s_saturation(spend_adstock, K_true[c], S_true[c])
...
def generate_sales_deterministic_weibull_s(spend, data, params):
    ...
    for l in range(L):
        for t in range(T):
            ...
            for c in range(C):
                ...
                spend_sat = weibull_s_saturation(spend_adstock, K[c], S[c])
                log_y += beta_media_loc[c, l] * spend_sat
...
def compute_true_roi_weibull_s(spend, data, params):
    """Compute true ROI using counterfactuals"""
    C, L, T = spend.shape
    roi_true = np.zeros(C)
    
    for c in range(C):
        sales_full = generate_sales_deterministic_weibull_s(spend, data, params)
        spend_cf = spend.copy()
        spend_cf[c, :, :] = 0
        sales_cf = generate_sales_deterministic_weibull_s(spend_cf, data, params)
...
TRUE_SATURATION = "weibull_sshape"

print("="*100)
print(f"MMM SATURATION FUNCTION SELECTION SIMULATION - TRUE DGP: Weibull S-shape")
print("="*100)
...
for scenario_name, scenario_config in scenarios.items():
    ...
    print(f"Ground Truth: Weibull S-shape saturation + adstock\n")
    ...
    S_true = np.random.uniform(1.5, 3.0, C)
    ...
    print(f"True S (shape, S>1): {S_true}")
    
    print("\nGenerating data...")
    spend, exog_data = generate_exogenous_data(C, L, T, seed=SEED)
    sales = generate_sales_weibull_s(spend, exog_data, true_params, seed=SEED)
    roi_true = compute_true_roi_weibull_s(spend, exog_data, true_params)
    ...
    models = create_model_specs_no_adstock(opt_tol, opt_maxiter, n_init, eta_sq)  # CHANGED from create_model_specs
    ...
    if len(valid_models) > 0:
        ...
        print("-"*100)
        print(f"Ground Truth: Weibull S-shape")
...
traditional_correct = df_all_selection[
    ...
    (df_all_selection['Selected_Model'] == 'Weibull S-shape')
].shape[0]
