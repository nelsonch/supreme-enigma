...
# Custom data generation for Hill S-shape
def generate_sales_hill_s(spend, data, params, seed=None):
    """Generate sales using Hill S-shape saturation + adstock"""
    if seed is not None:
        np.random.seed(seed)
    ...
    for l in range(L):
        for t in range(T):
            ...
            for c in range(C):
                ...
                spend_sat = hill_s_saturation(spend_adstock, K_true[c], S_true[c])
...
def generate_sales_deterministic_hill_s(spend, data, params):
    ...
    for l in range(L):
        for t in range(T):
            ...
            for c in range(C):
                ...
                spend_sat = hill_s_saturation(spend_adstock, K[c], S[c])
                log_y += beta_media_loc[c, l] * spend_sat
...
def compute_true_roi_hill_s(spend, data, params):
    """Compute true ROI using counterfactuals"""
    C, L, T = spend.shape
    roi_true = np.zeros(C)
    
    for c in range(C):
        sales_full = generate_sales_deterministic_hill_s(spend, data, params)
        spend_cf = spend.copy()
        spend_cf[c, :, :] = 0
        sales_cf = generate_sales_deterministic_hill_s(spend_cf, data, params)
...
TRUE_SATURATION = "hill_sshape"

print("="*100)
print(f"MMM SATURATION FUNCTION SELECTION SIMULATION - TRUE DGP: Hill S-shape")
print("="*100)
...
for scenario_name, scenario_config in scenarios.items():
    ...
    print(f"Ground Truth: Hill S-shape saturation + adstock\n")
    ...
    sales = generate_sales_hill_s(spend, exog_data, true_params, seed=SEED)
    roi_true = compute_true_roi_hill_s(spend, exog_data, true_params)
    ...
    if len(valid_models) > 0:
        ...
        print("-"*100)
        print(f"Ground Truth: Hill S-shape")
...
traditional_correct = df_all_selection[
    ...
    (df_all_selection['Selected_Model'] == 'Hill S-shape')
].shape[0]
