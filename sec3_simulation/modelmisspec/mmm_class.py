...
class MMM:
    def __init__(self, spec_name, saturation_type, 
                 opt_tol=1e-6, opt_maxiter=1000, n_init=5, eta_sq=1.0):
        self.spec_name = spec_name
        self.saturation_type = saturation_type  # 'hill_c', 'hill_s', 'weibull_c', 'weibull_s', 'sigmoid', 'erf'
    ...
    def sigmoid_saturation(self, z, K):
        """Sigmoid saturation"""
        return 1 / (1 + np.exp(-K * (z - 5000)))
    
    def erf_saturation(self, z, K):
        """Error function saturation"""
        from scipy.special import erf
        return 0.5 * (1 + erf((z - 5000) / K))
    
    def apply_saturation(self, z, K, S=None):
        ...
        elif self.saturation_type == 'sigmoid':
            return self.sigmoid_saturation(z, K)
        elif self.saturation_type == 'erf':
            return self.erf_saturation(z, K)
        else:
            raise ValueError(f"Unknown saturation type: {self.saturation_type}")
    
    def transform_media(self, spend, alpha, K, S=None):
        ...
        for c in range(C):
            for l in range(L):
                ...
                # Saturation
                if self.saturation_type in ['hill_c', 'hill_s', 'weibull_c', 'weibull_s']:
                    transformed[c, l, :] = self.apply_saturation(x_ad, K[c], S[c])
                else:  # sigmoid, erf
                    transformed[c, l, :] = self.apply_saturation(x_ad, K[c])
    ...
    def fit(self, spend, sales, exog):
        ...
        n_params = 1 + (L-1) + C*L + C + C  # beta_0, locations, media, alpha, K
        if self.saturation_type in ['hill_c', 'hill_s', 'weibull_c', 'weibull_s']:
            n_params += C  # S
        ...
        for init_idx in range(self.n_init):
            ...
            if self.saturation_type in ['hill_c', 'hill_s', 'weibull_c', 'weibull_s']:
                ...
                p0.extend(np.random.uniform(2000, 8000, C))  # K
...
class MMM_NoAdstock:
    ...
    def transform_media(self, spend, K, S=None):
        """Apply saturation ONLY (no adstock)"""
        C, L, T = spend.shape
        transformed = np.zeros((C, L, T))
        
        for c in range(C):
            for l in range(L):
                # NO adstock - directly apply saturation to raw spend
                if self.saturation_type in ['hill_c', 'hill_s', 'weibull_c', 'weibull_s']:
                    transformed[c, l, :] = self.apply_saturation(spend[c, l, :], K[c], S[c])
                else:  # sigmoid, erf
                    transformed[c, l, :] = self.apply_saturation(spend[c, l, :], K[c])
        
        return transformed
    
    def neg_log_likelihood(self, params, spend, sales, exog):
        """Negative log-likelihood (no alpha parameters)"""
        C, L, T = spend.shape
        
        # Unpack parameters (NO ALPHA)
        idx = 0
        beta_0_est = params[idx]; idx += 1
        beta_loc_est = np.concatenate([[0], params[idx:idx+L-1]]); idx += L-1
        ...
        # Transform media (no adstock)
        media_transformed = self.transform_media(spend, K_est, S_est)
    ...
    def fit(self, spend, sales, exog):
        """Fit model with multiple initializations (no alpha estimation)"""
        C, L, T = spend.shape
        
        # Initialize parameters (NO ALPHA)
        n_params = 1 + (L-1) + C*L + C  # beta_0, locations, media, K (no alpha)
        if self.saturation_type in ['hill_c', 'hill_s', 'weibull_c', 'weibull_s']:
            n_params += C  # S
        n_params += 10  # controls + sigma
        ...
        for init_idx in range(self.n_init):
            # Random initialization (NO ALPHA)
            p0 = []
            p0.append(np.random.randn())  # beta_0
            p0.extend(np.random.randn(L-1) * 0.1)  # locations
            p0.extend(np.random.uniform(0.1, 2, C*L))  # media
    ...
    def compute_roi(self, spend, exog):
        """Compute ROI via counterfactuals (no adstock)"""
        C, L, T = spend.shape
        roi = np.zeros(C)
