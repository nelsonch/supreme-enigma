...
# Custom data generation for Erf
def generate_sales_erf(spend, data, params, seed=None):
    """Generate sales using Erf saturation + adstock"""
    if seed is not None:
        np.random.seed(seed)
    ...
    for l in range(L):
        for t in range(T):
            ...
            for c in range(C):
                ...
                spend_sat = erf_saturation(spend_adstock, K_true[c])
...
def generate_sales_deterministic_erf(spend, data, params):
    ...
    for l in range(L):
        for t in range(T):
            ...
            for c in range(C):
                ...
                spend_sat = erf_saturation(spend_adstock, K[c])
                log_y += beta_media_loc[c, l] * spend_sat
...
def compute_true_roi_erf(spend, data, params):
    """Compute true ROI using counterfactuals"""
    C, L, T = spend.shape
    roi_true = np.zeros(C)
    
    for c in range(C):
        sales_full = generate_sales_deterministic_erf(spend, data, params)
        spend_cf = spend.copy()
        spend_cf[c, :, :] = 0
        sales_cf = generate_sales_deterministic_erf(spend_cf, data, params)
...
print("="*100)
print(f"MMM SATURATION FUNCTION SELECTION SIMULATION - TRUE DGP: Erf")
print("="*100)
...
for scenario_name, scenario_config in scenarios.items():
    ...
    print(f"Ground Truth: Erf saturation + adstock\n")
    ...
    # Erf: only K parameter (no S)
    K_true = np.random.uniform(2000, 8000, C)  # Spread parameter
    ...
    print(f"True K (spread): {K_true}")
    
    print("\nGenerating data...")
    spend, exog_data = generate_exogenous_data(C, L, T, seed=SEED)
    sales = generate_sales_erf(spend, exog_data, true_params, seed=SEED)
    roi_true = compute_true_roi_erf(spend, exog_data, true_params)
...
traditional_correct = df_all_selection[
    ...
    (df_all_selection['Selected_Model'] == 'Erf')
].shape[0]
